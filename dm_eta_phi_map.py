...
events = uproot.open("/ceph/mgais/SVJ_std2_UL2018_scouting_truth_study_inv_parts/SVJ_mMed-%sGeV_mDark-20GeV_rinv-%s_alpha-peak_13TeV/SVJ_mMed-%s_mDark-20_rinv-%s_alpha-peak.root"%(mass,rinv,mass,rinv))['mmtree/Events']

vector.register_awkward()
PFcands = ak.zip({
    "pt": np.concatenate(events['PFCands_pt'].array(),events['DarkMatterParticles_pt'].array()),
    "eta": np.concatenate(events['PFCands_eta'].array(),events['DarkMatterParticles_eta'].array()),
    "phi": np.concatenate(events['PFCands_phi'].array(),events['DarkMatterParticles_phi'].array()),
    "mass": np.concatenate(events['PFCands_mass'].array(),events['DarkMatterParticles_mass'].array()),
...
pfcand_pt = np.concatenate(events['PFCands_pt'].array(),events['DarkMatterParticles_pt'].array())[nFatJet][pt_cut]
pfcand_eta =  np.concatenate(events['PFCands_eta'].array(),events['DarkMatterParticles_eta'].array())[nFatJet][pt_cut]
pfcand_phi =  np.concatenate(events['PFCands_phi'].array(),events['DarkMatterParticles_phi'].array())[nFatJet][pt_cut]
...
for n in range(100):
    ...
    if matched < 2:
        ...
        fig.savefig(f'/web/mgais/public_html/scouting_truth_study/dm_pfcand_eta_phi/eta_phi_mZ-{mass}_rinv-{rinv}_ptcut-{ptcut}_event-{n}.png',bbox_inches='tight',dpi=300)
        fig.savefig(f'/web/mgais/public_html/scouting_truth_study/dm_pfcand_eta_phi/eta_phi_mZ-{mass}_rinv-{rinv}_ptcut-{ptcut}_event-{n}.pdf',bbox_inches='tight')
        plt.close()
